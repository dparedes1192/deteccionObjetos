...
if __name__ == "__main__":
    ...
    while cap:
        ...
        if ret is False:
            break
        frame = cv2.resize(frame, (900, 660), interpolation=cv2.INTER_CUBIC)
        ...
        for detection in detections:
            if detection is not None:
                ...
                for x1, y1, x2, y2, conf, cls_conf, cls_pred in detection:
                    ...
                    #frame = cv2.rectangle(frame, (x1, y1 + box_h), (x2, y1), color, -1)
                    c1 = x1, y1
                    c2 = x2, y2

                    cv2.rectangle(frame,(int(c1[0]), int(c1[1])), (int(c2[0]), int(c2[1])),color,5)
                    # Displaying the image
                    cv2.putText(frame, classes[int(cls_pred)], (int(c2[0])-100, int(c1[1])-25), cv2.FONT_HERSHEY_SIMPLEX, 1, color, 5)
                    cv2.putText(frame, str("%.2f" % float(conf)), (int(c1[0]), int(c1[1])), cv2.FONT_HERSHEY_SIMPLEX, 0.5,color, 5)
